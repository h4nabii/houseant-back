...
public class HouseOwnerController {
    ...
    public ResponseEntity<String> addHouseInfo(@RequestBody House newHouse, @NonNull HttpServletRequest request) {
    ...
    public ResponseEntity<String> updateHouseInfo(@RequestBody House newHouse, @NonNull HttpServletRequest request) {
        newHouse.setAccount(((User) request.getSession().getAttribute("user")).getAccount());
    ...
    public ResponseEntity<Map<String, Object>> deleteHouseInfo(@RequestParam int id,
            @NonNull HttpServletRequest request) {
        if (houseService.findByKey(Map.of(
                "account", ((User) request.getSession().getAttribute("user")).getAccount(),
                "id", id)) == null) {
            return ResponseEntity.ok()
                    .body(Map.of("message", "There is no listing information or the user is incorrect"));
